...
- Progressive incremental sync: первая пачка до 50 новых писем сразу отдаётся в UI, фоновая догрузка продолжается до лимита 200. Повторный sync сравнивает UID/UIDVALIDITY, загружает body только новых писем, отдельно обновляет Seen/Flagged и удаляет исчезнувшие на сервере UID.
...
- Временная ошибка IMAP отображается у конкретного аккаунта с действием «Повторить»; кэш остаётся доступен, а синхронизация других аккаунтов продолжается.
