...
export function safeEmail(html: string, images = false) {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src ${images ? "https: http:" : "'none'"}; style-src 'unsafe-inline'; base-uri 'none'; form-action 'none'"><meta name="referrer" content="no-referrer"><style>html,body{background:#1b1e2a;color-scheme:dark}body{color:#d0d3e0;font:15px/1.8 -apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;margin:0;padding:6px 0;overflow-wrap:anywhere}h1,h2,h3{color:#eef0fa;line-height:1.4}a{color:#9998ff}img{max-width:100%;height:auto}table{max-width:100%;border-collapse:collapse}td,th{padding:6px}blockquote{border-left:2px solid #555676;margin-left:0;padding-left:18px;color:#969aae}pre{white-space:pre-wrap}hr{border:0;border-top:1px solid #303342}</style></head><body>${doc.body.innerHTML}</body></html>`;
}
